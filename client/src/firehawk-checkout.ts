...
export async function verifyCheckoutReturn(identity: CheckoutIdentity): Promise<{ state: 'none' | 'cancelled' | 'completed' | 'pending'; reference?: string }> {
  ...
  for (let attempt = 0; attempt < 8; attempt += 1) {
    ...
    if (response.ok) {
      const result = await response.json() as { status?: string; reference?: string };
      if (result.status === 'completed') return { state: 'completed', reference: result.reference };
    }
